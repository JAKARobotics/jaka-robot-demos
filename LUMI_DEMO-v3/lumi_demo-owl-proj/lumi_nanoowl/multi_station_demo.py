...
import QR_SCAN
# 从配置文件加载站点配置
USER_CMD_CONFIG_PATH = './conf/userCmdControl-doll.json'

def load_stations():
    """从配置文件加载站点配置"""
    try:
        with open(USER_CMD_CONFIG_PATH, 'r') as f:
...
def update_operation_mode(station_config):
    ...
    try:
        ...
        with open(USER_CMD_CONFIG_PATH, 'w') as f:
...
def execute_task_at_station(control, station_id, station_config, tag=None):
    """
    在指定站点执行任务
    ...
    try:
        ...
        # 如果未传入tag，则用相机扫码获取
        if tag is None:
            tag = QR_SCAN.get_qr_text_from_camera()
        lumi_det_demo.run_detection(robot=control, auto_execute=True, tags=[tag] if tag else None)
...
def main():
    ...
    try:
        ...
        for station_id, station_config in stations.items():
            ...
            tag = "海绵宝宝"
            print(f"二维码内容: {tag}")
            execute_task_at_station(control, station_id, station_config, tag=tag)

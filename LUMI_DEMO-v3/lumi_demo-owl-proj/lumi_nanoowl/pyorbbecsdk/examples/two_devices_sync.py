...
def rendering_frames():
    ...
    saved = [False] * curr_device_cnt  # 新增：每台相机只保存一次
    ...
    while not stop_rendering:
        for i in range(curr_device_cnt):
            ...
            if color_frame is not None:
                ...
                # ====== 新增：保存彩色图片 ======
                if not saved[i]:
                    filename = f"device_{serial_number_list[i]}_color.jpg"
                    cv2.imwrite(filename, color_image)
                    print(f"已保存相机{i}（SN:{serial_number_list[i]}）的彩色图片到 {filename}")
                    saved[i] = True
                # =============================
            ...
            if depth_frame is not None:
                ...
                # 新增：保存深度图片
                if depth_image is not None and not saved[i]:
                    filename = f"device_{serial_number_list[i]}_depth.jpg"
                    cv2.imwrite(filename, depth_image)
                    print(f"已保存相机{i}（SN:{serial_number_list[i]}）的深度图片到 {filename}")
                    saved[i] = True

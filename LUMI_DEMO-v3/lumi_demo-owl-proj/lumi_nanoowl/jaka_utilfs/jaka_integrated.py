...
class JAKAIntegrated(JAKA):
    ...
    def _load_ext_axis_limits(self):
        ...
        try:
            import json
            with open('./conf/userCmdControl-doll.json', 'r') as f:
                config = json.load(f)
            if "extAxisLimits" in config:
                ...
                return {
                    "joint1": {"min": 0, "max": 270, "desc": "升降，单位mm"}, 
    ...
    def gripper_init(self):
        ...
        try:
            ...
            # 配置TIO接口参数
            self.robot.set_tio_vout_param(1, 0)  # 启用TIO，24V输出
            self.robot.set_tio_pin_mode(2, 1)    # 设置AI2为RS485L模式
            time.sleep(1)

            # 配置RS485通道
            self.robot.set_rs485_chn_mode(1, 0)  # 通道1为Modbus RTU模式
            ret = self.robot.set_rs485_chn_comm({
                'chn_id': 1,
                'slave_id': 1,
                'baudrate': 115200,
                'databit': 8,
            ...
            # 发送夹爪初始化命令
            init_command = bytearray.fromhex("01 06 01 00 00 01")
            ret = self.robot.send_tio_rs_command(1, init_command)
            print(f"夹爪初始化命令返回: {ret}")
            time.sleep(2)
    ...
    def gripper_open(self):
        ...
        try:
            print("正在打开夹爪...")
            # 发送夹爪打开命令
            open_command = bytearray.fromhex("01 06 01 03 01 F4")
            ret = self.robot.send_tio_rs_command(1, open_command)
            print(f"夹爪打开命令返回: {ret}")
            time.sleep(2)
            print("夹爪已打开")
            return True
    ...
    def gripper_close(self):
        ...
        try:
            print("正在关闭夹爪...")
            # 发送夹爪关闭命令
            close_command = bytearray.fromhex("01 06 01 03 00 00")
            ret = self.robot.send_tio_rs_command(1, close_command)
            print(f"夹爪关闭命令返回: {ret}")
            time.sleep(2)
            print("夹爪已关闭")
            return True
    ...
    def grab_action(self, send_tio):
        """
        重写父类的grab_action方法，使用新的夹爪控制方法
        :param send_tio: 0=打开夹爪，1=关闭夹爪
        :return: 成功返回True，失败返回False
        """
        if send_tio == 0:
            # 打开夹爪
    ...
    def setup_system(self):
        ...
        return robot_ok and ext_ok and gripper_ok
